...
func VRChatScreen(win fyne.Window) fyne.CanvasObject {
	...
	img, _ := fyne.LoadResourceFromURLString("https://cdn.discordapp.com/attachments/886109123492581486/886109823735849000/static.png")
	image := canvas.NewImageFromResource(img)
	image.FillMode = canvas.ImageFillStretch
	label := widget.NewLabel("Nothing to see here yet. To search select your search type,\n" +
		"input your query into the search box and then click search!\n" +
		"If you experience any issues, please reach out to top.\n\n" +
		"Thank you for using Eternity :)")
	...
	t := widget.NewToolbar(
		...
		widget.NewToolbarAction(theme.ContentCopyIcon(), func() {
			err := clipboard.WriteAll(label.Text + "\n" + clip)
			if err != nil {
				return
			}
		}),
	...
	hbox := container.NewVBox(t, label)
	...
	DisplayGroup := container.NewVSplit(image, container.NewCenter(hbox))
	ListGroup := container.NewHSplit(list, DisplayGroup)
